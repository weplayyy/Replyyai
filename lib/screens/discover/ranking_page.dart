...
class _RankingPageState extends State<RankingPage> {
      ...
      builder: (_) => AlertDialog(
        ...
        actions: [
          TextButton(
              ...
              child: const Text('Got it', style: TextStyle(color: Color(0xffbf70ff)))),
        ],
      ),
    );
  }
  ...
  Widget _titleHeader() {
    return SizedBox(
      ...
      child: Stack(alignment: Alignment.center, children: [
        ..._sparklePositions().map((p) => Positioned(
              left: MediaQuery.of(context).size.width * p[0],
              top: 72 * p[1],
              child: _SparkleWidget(size: p[2]),
            )),
        Positioned(left: 28, child: _LaurelWreath(mirrored: false)),
        Positioned(right: 28, child: _LaurelWreath(mirrored: true)),
        ShaderMask(
  ...
  List<List<double>> _sparklePositions() => [
    [0.12, 0.1, 10], [0.10, 0.8, 8], [0.82, 0.15, 9],
    [0.86, 0.75, 11], [0.5, 0.05, 7], [0.35, 0.9, 6], [0.65, 0.85, 8],
  ];
          ...
          return GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                gradient: sel
                    ? LinearGradient(colors: [accent.withOpacity(0.7), const Color(0xff4c1a87)])
                    : null,
              ...
              child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                Text(tabs[i].$1, style: const TextStyle(fontSize: 18)),
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            child: Row(children: [
              ...
              Text(_region, style: const TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w600)),
    ...
    showModalBottomSheet(
      ...
      builder: (_) => Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        ...regions.map((r) => ListTile(
          leading: Icon(r == 'Global' ? Icons.language : Icons.location_on,
          ...
          trailing: _region == r ? const Icon(Icons.check, color: Color(0xffbf70ff)) : null,
          onTap: () { setState(() => _region = r); Navigator.pop(context); },
        )),
  ...
  Widget _roomsBody() {
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      stream: FirebaseFirestore.instance.collection('rooms').where('status', isEqualTo: 'live').orderBy('listenerCount', descending: true).limit(50).snapshots(),
      builder: (context, snap) {
        if (!snap.hasData) return const Center(child: CircularProgressIndicator(color: Color(0xffCE93D8)));
        final all = snap.data!.docs.map((d) => _RoomEntry.fromMap(d.data(), d.id)).toList();
        ...
        children: List.generate(items.length, (i) {
          ...
          return GestureDetector(
            ...
            child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              Icon(items[i].$1,
                  color: sel ? const Color(0xffbf70ff) : Colors.white.withOpacity(0.35),
                  size: 28),
              const SizedBox(height: 5),
              Text(items[i].$2,
                  style: TextStyle(
                      color: sel ? Colors.white : Colors.white.withOpacity(0.38),
                      fontSize: 13)),
            ]),
          );
        }),
...
class _LaurelWreath extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    Widget w = CustomPaint(size: const Size(60, 60), painter: _LaurelPainter());
    if (mirrored) {
      w = Transform(alignment: Alignment.center, transform: Matrix4.rotationY(math.pi), child: w);
    }
    return w;
  }
}
...
class _LaurelPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final stemPath = Path()
      ...
      ..quadraticBezierTo(size.width * 0.2, size.height * 0.7, size.width * 0.05, size.height * 0.1);
    canvas.drawPath(stemPath, stem);

    for (final l in [
      [0.62, 0.85, -0.3], [0.5, 0.68, -0.5], [0.38, 0.52, -0.7],
      [0.25, 0.36, -0.9], [0.14, 0.22, -1.1],
    ]) {
      canvas.save();
      ...
      canvas.drawPath(Path()
        ...
        ..quadraticBezierTo(-10, 2, 0, 0), fill);
      canvas.restore();
    }

    canvas.drawCircle(Offset(size.width * 0.05, size.height * 0.08), 4,
      ...
      child: Row(children: [
        ...
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text('💎 ${entry.score(filter)} hearts',
  ...
  Widget _twoAvatars() {
    return SizedBox(
      ...
      child: Stack(children: [
        ...
        Positioned(left: 20, child: _av(entry.user2Photo, 'B', const Color(0xffE040FB))),
      ]),
    );
  }

  Widget _av(String? photo, String fb, Color color) {
    return Container(
      width: 36, height: 36,
      decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: color, width: 2)),
      child: ClipOval(
        child: photo != null && photo.isNotEmpty
            ? Image.network(photo, fit: BoxFit.cover, errorBuilder: (_, __, ___) => _fallback(fb, color))
            : _fallback(fb, color),
      ),
    );
  }

    Widget _fallback(String t, Color c) => Container(
...
class _ClanRow extends StatelessWidget {
        ...
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(entry.name, maxLines: 1, overflow: TextOverflow.ellipsis,
                style: const TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w700)),
            Text('👥 ${entry.members} members · 💎 ${entry.score(filter)} coins',
                style: TextStyle(color: Colors.white.withOpacity(0.55), fontSize: 12)),
          ]),
        ),
  ...
  Widget _fallback(String name, Color c) => Container(
    color: c.withOpacity(0.3),
...
Widget _rankNum(int rank) {
  ...
  return SizedBox(
    ...
    child: Text('$rank',
        ...
        style: TextStyle(
            color: color, fontSize: rank <= 3 ? 18 : 15, fontWeight: FontWeight.w900)),
  );
}
  ...
  Widget build(BuildContext context) {
    ...
    return SizedBox(
      ...
      child: Stack(alignment: Alignment.bottomCenter, children: [
        Positioned(left: 18, bottom: 8,
            child: _WinnerCard(rank: 2, entry: padded[1], color: const Color(0xff2298ff), height: 235, filter: filter, onTap: onTap)),
        Positioned(bottom: 0,
            child: _WinnerCard(rank: 1, entry: padded[0], color: const Color(0xffffb22b), height: 285, isFirst: true, filter: filter, onTap: onTap)),
        Positioned(right: 18, bottom: 8,
            child: _WinnerCard(rank: 3, entry: padded[2], color: const Color(0xffc250ff), height: 235, filter: filter, onTap: onTap)),
      ]),
    );
  }
}
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: isEmpty ? [] : [BoxShadow(color: color.withOpacity(0.55), blurRadius: 24, spreadRadius: 1)],
        ),
        child: Stack(alignment: Alignment.topCenter, clipBehavior: Clip.none, children: [
          ...
          Positioned(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                boxShadow: isEmpty ? [] : [BoxShadow(color: color.withOpacity(0.8), blurRadius: 18)],
              ),
              child: ClipOval(
                child: photo != null && photo.isNotEmpty
                    ? Image.network(photo, fit: BoxFit.cover,
          ...
          Positioned(
            top: isFirst ? 172 : 138, left: 8, right: 8,
            child: Column(children: [
              Text(name, maxLines: 1, overflow: TextOverflow.ellipsis,
                  textAlign: TextAlign.center,
                  style: TextStyle(
                      color: isFirst ? const Color(0xffffe45c) : Colors.white.withOpacity(0.95),
                      fontSize: isFirst ? 20 : 16, fontWeight: FontWeight.w900)),
              const SizedBox(height: 4),
              if (!isEmpty && rankInfo.info.tier != RankTier.none)
                RankBadge(charms: entry.charms, scale: isFirst ? 0.75 : 0.65),
              const SizedBox(height: 6),
              if (!isEmpty) ...[
                Text('Charm', style: TextStyle(color: Colors.white.withOpacity(0.7), fontSize: 13)),
        ...
        if (entry != null) ...[
          ...
          Text(_fmt(entry!.score(filter)),
              style: const TextStyle(
                  color: Color(0xffbf70ff), fontSize: 15, fontWeight: FontWeight.w900)),
        ] else
          Expanded(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            gradient: RadialGradient(
              colors: [const Color(0xff6d28d9).withOpacity(0.35), Colors.transparent],
            ),
          ),
        ),
      ),
